...
Ws = r2_lower_bound;        % stopband edge frequency (Hz)
Ws = 2 * Ws / F_s;          % normalized stopband edge frequency (*pi)

[nBlockD, ~] = ellipord(Wp, Ws, Rp, Rs);
[numBlockD, denBlockD] = ellip(nBlockD, Rp, Rs, Wp, 'low');
...
Ws = signal_bw * 2 / F_s;       % Stopband corner frequency (* pi)
Rp = 0.99 / ripple2;            % Passband ripple
Rp = -20 * log10(Rp);           % Passband ripple in decibels
Rs = 0.001;                     % Stopband attenuation
Rs = -20 * log10(Rs);           % Stopband attenuation in decibels

[nBlockB, ~] = ellipord(Wp, Ws, Rp, Rs);
[numBlockB, denBlockB] = ellip(nBlockB, Rp, Rs, Wp, 'high');
